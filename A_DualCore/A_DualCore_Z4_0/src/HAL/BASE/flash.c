...
#define CCP_DATA_STAT_ADDR		0x820000
#define CCP_DATA_SIZE			0xC300		//50K-0x500
...
uint32_t FLASH_EE_Write8(uint32_t address, uint32_t length, uint8_t *data)
{
    ...
    if( ((address + length) <= CCP_DATA_STAT_ADDR) || ((CCP_DATA_STAT_ADDR + CCP_DATA_SIZE) <= address) )	//添加标定区间的写保护
    {
