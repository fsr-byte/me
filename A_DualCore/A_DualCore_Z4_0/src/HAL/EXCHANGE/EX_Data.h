...
//NVM
__EX_DATA_EXTERN__ B_DHP_model_T DHP_model_B;
__EX_DATA_EXTERN__ APPDID_outSnapshots appDID_outSnapshot;
__EX_DATA_EXTERN__ uint8_t VHALI_TBOX_TimeYear;
__EX_DATA_EXTERN__ uint8_t VHALI_TBOX_TimeMonth;
__EX_DATA_EXTERN__ uint8_t VHALI_TBOX_TimeDate;
__EX_DATA_EXTERN__ uint8_t VHALI_TBOX_TimeHour;
__EX_DATA_EXTERN__ uint8_t VHALI_TBOX_TimeMinute;
__EX_DATA_EXTERN__ uint8_t VHALI_TBOX_TimeSecond;
__EX_DATA_EXTERN__ uint16_t VHALI_BMSErrNum;
__EX_DATA_EXTERN__ uint8_t VHALI_MCUErrNum;
__EX_DATA_EXTERN__ uint8_t Fault_storage_array[2480];  /*bug 1515: change snapshot size to 62 bytes*/
__EX_DATA_EXTERN__ uint8_t DTCStatusRAMTbl[UDS_SP_DTC_NUM];
__EX_DATA_EXTERN__ PMM PMM_output;//需要其中的两个量：WakeSrcValid 、VPMM_InternalSleepActive_flg
__EX_DATA_EXTERN__ uint16_t VHALO_MotTqReq;
//UDS
__EX_DATA_EXTERN__ uint16_t VHALI_ABS_VehicleSpeed;
///__EX_DATA_EXTERN__ uint16_t VHALI_MCUElecMotSpd;
__EX_DATA_EXTERN__ int16_t VINP_MCUElecMotSpd_rpm; //bug 1620
__EX_DATA_EXTERN__ IO_INPUT_Rx_32B_tag	FS6500_IO_Input;

/*-----------NVM UDS输出，从Core0到Core1 -----------------*/
//NVM
extern uint8_t gbCalDataCorrect;
extern uint8_t gbDidDataCorrect;
extern uint8_t seriousHistFaultFlag;
extern uint8_t HistDataCheckFaliedFlag;
extern uint8_t AppDataCheckFaliedFlag;
extern uint8_t DtcToCAN[DTC_NUM_IN_RAM];
extern NvMLoadSaveType NvMAppDataSaveLoadFlag;
extern boolean VBTK_SleepAllw_flg;
//UDS
extern udsIOCTRLDIDType IOCtrlDidTbl[3];  /*task.c 使用 */
///extern boolean DisRecordDTC;           /*DHP 使用，表示是否关闭诊断 */

extern uint8_t PEPS_FixedCode_Byte[4];
extern uint8_t PEPS_VSK_Byte[16];
extern uint8_t ECU_FUN_CFG_Byte[6];   //ECU configuration(DID: F1FA)
extern uint8_t ECU_FUN_CFG_Valid;
extern uint8_t VCU_CCP_Byte[1];
extern uint8_t VCU_CCP_Valid;
extern uint8_t VCU_CFG_Byte[8];   //VCU configuration(DID: F1FD)
extern uint8_t VCU_CFG_Valid;

extern uint8_t VHALO_VCUHwVer1;
extern uint8_t VHALO_VCUHwVer2;
extern uint16_t VHALO_VCUSwVer1;
extern uint8_t VHALO_VCUSwVer2;
extern uint8_t VHALO_VCUSwVer3;
extern Config Config_output;
...
typedef struct _Exchange_C1T0_struct_tag_{
  ...
  //uint8_t Fault_storage_array[1830];
...
typedef _Exchange_C0T1_tag_ 	_Exchange_Output_tag_;//----不同核时需要改变
typedef _Exchange_C1T0_tag_ 	_Exchange_Input_tag_;//----不同核时需要改变
