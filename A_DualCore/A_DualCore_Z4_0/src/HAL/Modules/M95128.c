...
uint32_t ReadExtEE(uint16_t addr, uint16_t size, uint8_t * data)
{
    ...
    uint16_t temp2 = 0;
    uint8_t IniStatus = 0;
    uint32_t recode = EXTEE_OK;

    if( (addr >= M95128_Capacity) || ((addr + size) > M95128_Capacity) || (size > M95128_Capacity) )
    {	//范围判断
    ...
    if( 0 != Ex_Base_LockGate(Gate_SPI) )	//锁定信号量失败
    {
        return EXTEE_SPIBUSY;
    }
    ...
    *(data) = temp;
    ...
    if( 0 != Ex_Base_UnlockGate(Gate_SPI) )
    {
        recode = EXTEE_UNLOCKERROR;
    }
    else
    {
        recode = EXTEE_OK;
